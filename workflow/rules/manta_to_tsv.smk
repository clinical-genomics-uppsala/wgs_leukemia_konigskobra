...
rule manta_to_tsv_indeldup:
    input:
        vcf="cnv_sv/manta_run_workflow_{analysis}/{sample}.ssa.vcf",
    output:
        dels=temp("tsv_files/{sample}_manta_{analysis}.del.tsv"),
        dups=temp("tsv_files/{sample}_manta_{analysis}.dup.tsv"),
        ins=temp("tsv_files/{sample}_manta_{analysis}.ins.tsv"),
    log:
        "tsv_files/{sample}_manta_{analysis}.indeldup.tsv.log",
    benchmark:
        repeat(
            "tsv_files/{sample}_manta_{analysis}.indeldup.tsv.benchmark.tsv",
            config.get("manta_to_tsv", {}).get("benchmark_repeats", 1),
        )
    threads: config.get("manta_to_tsv", {}).get("threads", config["default_resources"]["threads"])
    resources:
    ...
    container:
        config.get("manta_to_tsv", {}).get("container", config["default_container"])
    message:
        "{rule}: select del, ind, and dup info from {input.vcf} without filtering for diagnostic regions and compile into tsv"
    script:
